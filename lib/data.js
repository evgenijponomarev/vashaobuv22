...
export function getBanners(storeCode) {
  const bannersDirPath = path.join(process.cwd(), 'public', 'banners');
  const allFileNames = fs.readdirSync(bannersDirPath);

  if (!storeCode) return allFileNames.map((fileName) => `/banners/${fileName}`);

  return allFileNames
    .filter((fileName) => fileName.includes(storeCode))
    .map((fileName) => `/banners/${fileName}`);
}
