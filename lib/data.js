...
export function getProductPhotoLinks(productCode) {
  return fsHelpers.getProductPhotos(productCode)
    .map((fileName) => `/shoes_photos/${fileName}`);
}
...
export function getContacts(storeCode) {
  return fsHelpers.getContacts()
    .find(({ store_code }) => store_code === storeCode);
}

export function getStorePhotos(storeCode) {
  return fsHelpers.getStorePhotos(storeCode).map((fileName) => `/stores_photos/${fileName}`);
}
