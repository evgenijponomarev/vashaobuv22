...
const API_PASSWORD_FILE_PATH = path.join(process.cwd(), API_PASSWORD_FILE_NAME);
const IMPORT_LOG_FILE_PATH = path.join(process.cwd(), IMPORT_LOG_FILE_NAME);
...
const getUploadMiddlware = (filename, fileNameHandler) => multer({
  storage: multer.diskStorage({
    destination: UPLOADS_DIR_PATH,
    filename: (req, file, cb) => {
      if (fileNameHandler) fileNameHandler(file.originalname);
      cb(null, filename);
    },
  }),
});
...
const saveSourceData = async (fileData) => {
  if (!fs.existsSync(UPLOADS_DIR_PATH)) fs.mkdirSync(UPLOADS_DIR_PATH);
  if (!fs.existsSync(SHOES_PHOTOS_DIR_PATH)) fs.mkdirSync(SHOES_PHOTOS_DIR_PATH);
